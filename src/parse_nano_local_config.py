...
_default_compose_path = os.path.join(_app_dir,
                                     "./config/default_docker-compose.yml")
...
class ConfigReadWrite:
    ...
    def __init__(self):
        if not os.path.exists("nano_local_config.toml"):
            logging.warning(
                "No config file exists. creating 'nano_local_config.toml'")
            subprocess.call(
                "cp -p nano_local_config.example.toml nano_local_config.toml",
                shell=True)
...
class Helpers:
    ...
    maketrans = bytes.maketrans if hasattr(bytes,
                                           'maketrans') else string.maketrans
...
class ConfigParser:
    ...
    def __init__(self, genesis_node_name="nl_genesis"):
        ...
        #set compose nodes and rpc_url in config_dict.representative.nodes.node_name.rpc_url
    ...
    def __set_node_accounts(self):
        ...
        for node in self.config_dict["representatives"]["nodes"]:
            ...
            if "vote_weight_percent" in node:
                node["balance"] = self.nt.raw_mul(available_supply,
                                                  node["vote_weight_percent"])
    ...
    def __set_special_account_data(self):
        ...
        self.config_dict["genesis_account_data"] = self.key_expand(
            self.config_dict["genesis_key"])
        self.config_dict["canary_account_data"] = self.key_expand(
    ...
    def __config_dict_set_node_variables(self):
        ...
        for node in self.config_dict["representatives"]["nodes"]:
            if "name" not in node:
                ...
                logging.warning(
                    f'no name set for a node. New name : {node["name"].lower()}'
                )
    ...
    def __set_balance_from_vote_weight(self):
        ...
        for node_conf in self.get_nodes_config():
            if "vote_weight" in node_conf:
                node_conf["balance"] = self.nt.raw_mul(
    ...
    def get_node_config(self, node_name):
        result = self.h.value_in_dict_array(
    ...
    def set_docker_compose_services(self, genesis_node_name):
        host_port_inc = 0
        for node in self.config_dict["representatives"]["nodes"]:
            self.compose_add_node(node["name"])
            self.compose_set_node_ports(node["name"], host_port_inc)
        ...
        if self.get_config_value("promexporter_enable"):
    ...
    def set_nanovotevisu_compose(self, genesis_node_name):
        ...
        self.compose_dict["services"]["nl_nanovotevisu"]["build"]["args"][
            1] = f'HOST_ACCOUNT={self.get_node_config(genesis_node_name)["account"]}'
    ...
    def set_nanolooker_compose(self):
        ...
        self.enabled_services.append(
            f'nanolooker enabled at {self.get_config_value("remote_address")}:42000'
        )
    ...
    def set_promexporter_compose(self):
        ...
        for node in self.config_dict["representatives"]["nodes"]:
            ...
            node_port = self.get_node_config(
                node["name"])["rpc_url"].split(":")[2]
    ...
    def compose_add_node(self, node_name):
        ...
        if self.get_representative_config(
                "docker_tag",
                node_name)["found"]:  #search by individual docker_tag
            #default_docker
            container = self.compose_add_container(node_name, "default_docker")
            docker_tag = self.get_representative_config(
                "docker_tag", node_name)["value"]
            container["image"] = f"{docker_tag}"

        elif self.get_representative_config(
                "nano_node_path",
                node_name)["found"]:  #search by individual nano_node_path
            #default_build
            container = self.compose_add_container(node_name, "default_build")
            dockerfile_path = self.cp_dockerfile_and_nano_node(
                self.get_representative_config("nano_node_path",
                                               node_name)["value"], node_name)
            container["build"] = f"{dockerfile_path}/."

        elif self.get_representative_config(
                "docker_tag", None)["found"]:  #search by shared docker_tag
            #default_docker
            container = self.compose_add_container(node_name, "default_docker")
            docker_tag = self.get_representative_config("docker_tag",
                                                        None)["value"]
            container["image"] = f"{docker_tag}"

        elif self.get_representative_config(
                "nano_node_path",
                None)["found"]:  #search by shared nano_node_path
            #default_build
            container = self.compose_add_container(node_name, "default_build")
            dockerfile_path = self.cp_dockerfile_and_nano_node(
                self.get_representative_config("nano_node_path",
                                               None)["value"], node_name)
            container["build"] = f"{dockerfile_path}/."
        else:
            container = self.compose_add_container(node_name, "default_docker")
    ...
    def compose_set_node_ports(self, node_name, port_i):
        ...
        self.compose_dict["services"][node_name]["ports"] = [
            f'{host_port_peer}:17075', f'{host_port_rpc}:17076',
            f'{host_port_ws}:17078'
        ]
    ...
    def compose_add_container(self, node_name, default_container):
        ...
        self.compose_dict["services"][node_name]["volumes"][
            0] = self.compose_dict["services"][node_name]["volumes"][
                0].replace(default_container, node_name)
    ...
    def get_representative_config(self, node_key, node_name):
        ...
        if node_name is None:
            ...
            #individual config
            representatives_config = self.h.value_in_dict_array(
                self.config_dict["representatives"]["nodes"], node_name)
