...
class BlockGenerator():
    ...
    def __init__(self, broadcast_blocks=False, default_rpc_index=0):
        ...
        self.nano_rpc_default = self.nano_rpc_all[min(
            default_rpc_index,
            len(self.nano_rpc_all) - 1)]

    def get_rpc_all(self):
        return [NanoRpc(x) for x in self.conf.get_rpc_endpoints()]

    def get_nano_rpc_default(self):
        return NanoRpc(self.conf.get_rpc_endpoints()[min(
            self.default_rpc_index,
            len(self.nano_rpc_all) - 1)])
    ...
    def blockgen_single_account_opener(self,
                                       representative,
                                       source_key,
                                       destination_seed,
                                       send_amount,
        ...
        destination = nano_rpc.generate_account(destination_seed,
                                                destination_index)

        send_block = nano_rpc.create_send_block_pkey(source_key,
                                                     destination["account"],
                                                     send_amount,
    ...
    def recursive_split(self, seed_prefix, representative, source_private_key,
                        number_of_accounts, splitting_depth, current_depth,
        ...
        blocks_current_depth = self.blockgen_single_account_opener(
            ...
            seed,
            int(
                self.nt.raw_mul((2**(splitting_depth - current_depth + 1) - 1),
                                final_account_balance_raw)),
            number_of_accounts,
    ...
    def blockgen_account_splitter(self,
        ...
        if current_depth == 1:
            ...
            if source_seed is None:  #find a seed with enough funding
                for node_conf in self.conf.get_nodes_config():
                    ...
                    if int(
                            nano_rpc.check_balance(
                                node_conf["account"])["balance_raw"]) > (
                                    number_of_accounts *
                                    final_account_balance_raw):  #raw
                ...
                source_account_data = nano_rpc.generate_account(
            ...
            unittest.TestCase().assertGreater(
                ...
                int(
                    self.nt.raw_mul(number_of_accounts,
                                    final_account_balance_raw)))
            representative = nano_rpc.account_info(
                source_account_data["account"]
            )  #keep the same representative for all opened accounts
        else:
        ...
        blocks_A = self.recursive_split(seed_prefix_A, representative,
                                        source_account_data["private"],
                                        number_of_accounts, splitting_depth,
                                        current_depth,
                                        final_account_balance_raw)
        blocks_B = self.recursive_split(seed_prefix_B, representative,
                                        source_account_data["private"],
                                        number_of_accounts, splitting_depth,
                                        current_depth,
                                        final_account_balance_raw)
...
class BlockAsserts():
    ...
    def assert_nanoticker_reader(self, ledger_block_count, exit_after_s=180):
        ...
        try:
            with timeout(exit_after_s, exception=RuntimeError):
                while True:
                    res = self.nano_rpc_default.request_get(
                        f'http://{self.conf.get_remote_address()}:42002/json/stats.json'
                    )
                    if res["status_code"] == 200:
                        if res["message"] is not None and "blockCountMin" in res[
                                "message"]:
    ...
    def assert_blocks_published(self, blocks, sync=True):
        ...
        res = self.nano_rpc_default.publish_blocks(
            blocks, json_data=True, sync=sync)  #we don't care about the result
        ...
        try:
            with timeout(exit_after_s, exception=RuntimeError):
                while True:
                    ...
                    if int(rpc_block_count_end["count"]) == expected_count:
                        break
                    time.sleep(0.2)
        except Exception as e:
            self.tc.fail(str(e))
        self.tc.assertGreaterEqual(
            int(rpc_block_count_end["count"]),
                ...
                while True:
                    ...
                    for nano_rpc in self.nano_rpc_all:
                        ...
                        equal_count = equal_count + (
                            block_count["count"] == block_count["cemented"])
    ...
    def network_status(self):
        ...
        for nano_rpc in self.nano_rpc_all:
            ...
            node_name = self.conf.map_rpc_endpoint_to_node_name(nano_rpc)
            nodes.append({
                "node_name": node_name,
...
class BlockReadWrite():
    ...
    def write_blocks_to_disk(self, rpc_block_list, path):
        ...
        if any(isinstance(i, list)
            ...
            seed_list = list(
                set([
                    x["account_data"]["source_seed"] for x in rpc_block_list
                    if x["account_data"]["source_seed"] is not None
