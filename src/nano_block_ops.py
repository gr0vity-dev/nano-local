...
class BlockAsserts():
    ...
    def assert_blocks_published(self, blocks, sync = True):
        ...
        self.tc.assertGreaterEqual(rpc_block_count_end - rpc_block_count_start, blocks_to_publish_count ) #if other blocks arrive in the meantime
        ...
        try:           
            ...
            while confirmed_count < block_count:
                ...
                if confirmed_count == last_confirmed_count : # stalling block_count                    
                    ...
                    timeout_inc = 0
                if timeout_max <= 0 : 
                    self.tc.fail(f"Max timeout of {timeout_max} seconds reached")
    ...
    def assert_all_blocks_cementing(self, max_timeout = 30):
        try:
            with timeout(max_timeout, exception=RuntimeError) :
                while True :
